  ...
  const validateCart = async (showNotifications = true) => {
    ...
    try {
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        ...
        // Server error - only show if manually triggered
        ...
        // Other HTTP errors - show specific message
      ...
      // If showNotifications is false, silently fail (automatic validation)
    } finally {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (!document.hidden && user_id && cartItems.length > 0 && localStorage.getItem('token')) {
        // Only validate if user has been away for more than 60 seconds
        const now = Date.now();
        if (now - lastValidationTime.current > 60000) {
          validateCart(false); // Don't show notifications for automatic validation
        }
      }
    };
